 ...
 * @see
 */
public class ReflexivDSMainMercy implements Serializable{
    private long time;
    ...
    public void assembly() {
        ...
        Dataset<Row> ReflexivLongFragmentDSWithMarker;
        StructType ReflexivLongKmerWithMarkerStruct = new StructType();
        ReflexivLongKmerWithMarkerStruct = ReflexivLongKmerWithMarkerStruct.add("k-1", DataTypes.LongType, false);
        ReflexivLongKmerWithMarkerStruct = ReflexivLongKmerWithMarkerStruct.add("reflection", DataTypes.IntegerType, false);
        ReflexivLongKmerWithMarkerStruct = ReflexivLongKmerWithMarkerStruct.add("extension", DataTypes.createArrayType(DataTypes.LongType), false);
        ReflexivLongKmerWithMarkerStruct = ReflexivLongKmerWithMarkerStruct.add("left", DataTypes.IntegerType, false);
        ReflexivLongKmerWithMarkerStruct = ReflexivLongKmerWithMarkerStruct.add("right", DataTypes.IntegerType, false);
        ReflexivLongKmerWithMarkerStruct = ReflexivLongKmerWithMarkerStruct.add("bridge", DataTypes.LongType, false);
        ExpressionEncoder<Row> ReflexivLongKmerWithMarkerEncoder = RowEncoder.apply(ReflexivLongKmerWithMarkerStruct);
        ...
        if (param.partitions > 0) {
            ContigDS = ContigDS.repartition(param.partitions);
        }

        DSContigStringToReflexivKmer contigStringToReflexivKmer = new DSContigStringToReflexivKmer();

        ReflexivLongSubKmerDS = ContigDS.mapPartitions(contigStringToReflexivKmer, ReflexivLongKmerEncoder);
        ...
        DSFragmentRedundantFilter RedundantFragmentFilter = new DSFragmentRedundantFilter();
        ...
        ReflexivLongFragmentDSWithMarker = FastqDS.mapPartitions(LowCoverageReadDetector, ReflexivLongKmerWithMarkerEncoder);
        ReflexivLongFragmentDSWithMarker = ReflexivLongFragmentDSWithMarker.sort("bridge");

        ReflexivLongFragmentDS = ReflexivLongFragmentDSWithMarker.mapPartitions(RedundantFragmentFilter, ReflexivLongKmerEncoder);
    ...
    class DSContigStringToReflexivKmer implements MapPartitionsFunction<String, Row>, Serializable{
        List<Row> ReflexivBinaryList = new ArrayList<Row>();
        String[] units;
        String contigID;
        String read;
        int readLength;
        ...
        public Iterator<Row> call (Iterator<String> sIterator){
            ...
            while(sIterator.hasNext()){
                ...
                int coverageMarkerStart = contigID.indexOf("(");
                int coverageMarkerEnd = contigID.indexOf(")");
                String[] contigKmerCoverage =  contigID.substring(coverageMarkerStart+1,coverageMarkerEnd).split(",");
                int leftCoverage = Integer.parseInt(contigKmerCoverage[0]);
                int rightCoverage = Integer.parseInt(contigKmerCoverage[1]);

                if (readLength - param.kmerSize <= 1 ) {
                    continue;
                }

                Row reflexivKmerFromContig = reflexivKmerExtractionFromLowCoverageFragment(read, leftCoverage, rightCoverage);
                ReflexivBinaryList.add(reflexivKmerFromContig);
            }

            return ReflexivBinaryList.iterator();
        }
        
        private Row reflexivKmerExtractionFromLowCoverageFragment(String s, int left, int right){
            int remain = s.length() - param.subKmerSize;
            long nucleotideBinary=0L;
            long[] remainNucleotidesArray = new long[(remain-1)/31+1];
            int firstBlock = (remain-1) % 31 + 1;
...
/*
            ...
            System.out.println();
*/
            // add contig index to the right coverage, now right coverage is currently positive

            return RowFactory.create(nucleotideBinary,
                    1, remainNucleotidesArray, left, right
            );
        }
    ...
    class DSLowCoverageSubKmerExtraction implements  MapPartitionsFunction<Row, Row>, Serializable{
        ...
        public Iterator<Row> call (Iterator<Row> sIterator){
            ...
            if (partID < 100){
                contigIndex =partID * 10000000;
            }else if (partID >=100 && partID < 1000){
                contigIndex = partID * 1000000;
            }else if (partID >= 1000 && partID < 10000) {
                contigIndex = partID * 100000;
            }else if (partID >= 10000){
                contigIndex = partID * 10000;
            }
                ...
                for (int i = 0; i < readLength; i++) {
                    ...
                    if (i >= param.subKmerSize - 1) {
                        if (probTable.containsKey(nucleotideBinary)){
                            marker = (probTable.get(nucleotideBinary)& maxContigIndexBits);
...
*/
    //                        System.out.println(" " + marker + " " + direction);
                ...
                if (probedLeftIndex >=0 && probedRightIndex >=0 && probedLeftIndex < probedRightIndex){
                    long bridge = ( ((long) marker << 32) | (long) probedContigID);
 ...
 //                   System.out.println("forward: " + lowCoverageFragment);
                    fragmentList.add(
                            reflexivKmerExtractionFromLowCoverageFragment(lowCoverageFragment, bridge)
                    );
                }
                ...
                for (int i = 0; i < readLength; i++) {
                    ...
                    if (i >= param.subKmerSize - 1) {
                        if (probTable.containsKey(nucleotideBinaryReverseComplement)){
                            marker = (probTable.get(nucleotideBinaryReverseComplement) & maxContigIndexBits);
...
*/
 ...
 //                   System.out.println("RC: " + lowCoverageRCFragment);
                    fragmentList.add(
                        reflexivKmerExtractionFromLowCoverageFragment(lowCoverageRCFragment, bridge)
                    );
                }
        ...
        private Row reflexivKmerExtractionFromLowCoverageFragment(String s, long l){
            int remain = s.length() - param.subKmerSize;
            long nucleotideBinary=0L;
            long[] remainNucleotidesArray = new long[(remain-1)/31+1];
            int firstBlock = (remain-1) % 31 +1;
...
*/
            return RowFactory.create(nucleotideBinary,
                    1, remainNucleotidesArray, -10000000, -10000000, l
            );
        }
    ...
    class DSFragmentRedundantFilter implements MapPartitionsFunction<Row, Row>, Serializable{
        List<Row> FilteredFragments = new ArrayList<Row>();
        ...
        public Iterator<Row> call(Iterator<Row> sIterator) {
            ...
            while (sIterator.hasNext()) {
                Row s = sIterator.next();
                if (s.getLong(5) == bridge){
                    continue;
                }else{
                    bridge = s.getLong(5);
                    FilteredFragments.add(
                            RowFactory.create(s.getLong(0),s.getInt(1),s.getSeq(2), s.getInt(3), s.getInt(4))
                    );
                }
            }

            return  FilteredFragments.iterator();
        }
    }
