 ...
 * @see
 */
public class ReflexivDSStitchingLonger implements Serializable {
    private long time;
    ...
    public void assemblyFromKmer() {
        ...
        Dataset<Row> ContigRowDS;

        Dataset<Row> StitchingKmer;
        Dataset<Row> StitchingContig;
        StructType ReflexivFixingKmerStruct = new StructType();
        ReflexivFixingKmerStruct= ReflexivFixingKmerStruct.add("k-1", DataTypes.createArrayType(DataTypes.LongType), false);
        ...
        ContigRowDS = spark.read().csv(param.inputKmerPath2);

        if (param.partitions > 0) {
            KmerCountDS = KmerCountDS.repartition(param.partitions);
            ContigRowDS = ContigRowDS.repartition(param.partitions);
        }
        ...
        StitchingKmer = KmerCountDS.mapPartitions(ReducedKmerToSubKmer, ReflexivFixingKmerEndocer);

        ContigBinarizerFromReducedToSubKmer ContigToSubKmer = new ContigBinarizerFromReducedToSubKmer();
        StitchingContig = ContigRowDS.mapPartitions(ContigToSubKmer, ReflexivFixingKmerEndocer);

        StitchingKmer = StitchingKmer.union(StitchingContig);

        DSExtendReflexivKmerToArrayLoopFirstForRightSide DSKmerExtenstionArrayToArrayFirstToChangeToRightSide = new DSExtendReflexivKmerToArrayLoopFirstForRightSide(); // first time it is Contig reflected, k-mer forward. Seconde time it is Contig forward, k-mer reflected, to avoid k-mer k-mer extension

        StitchingKmer = StitchingKmer.sort("k-1");
        StitchingKmer = StitchingKmer.mapPartitions(DSKmerExtenstionArrayToArrayFirstToChangeToRightSide, ReflexivFixingKmerEndocer);
        ...
        while (iterations <= param.maximumIteration) {
            iterations++;
            if (iterations >= param.minimumIteration + 3 + 22) { // 15 + 3 is 18. for reduced iterations
                break;
            }else {
                StitchingKmer = StitchingKmer.sort("k-1");
                StitchingKmer = StitchingKmer.mapPartitions(DSKmerExtenstionArrayToArray, ReflexivFixingKmerEndocer);
            }
        }

        StitchingKmer.persist(StorageLevel.MEMORY_AND_DISK());

        DSBinaryFixingKmerToFullKmer FixingKmer2FullKmer = new DSBinaryFixingKmerToFullKmer();

        ContigDS = StitchingKmer.mapPartitions(FixingKmer2FullKmer, Encoders.STRING());
    ...
    class DSBinaryFixingKmerToFullKmer implements MapPartitionsFunction<Row, String>, Serializable{
        ...
        public Iterator<String> call(Iterator<Row> sIterator) throws Exception {
            while (sIterator.hasNext()) {
                ...
                reflexivKmerStringList.add(
                        BinaryBlocksToString(combinedArray)
                );
            }
...
//                new Tuple2<Long, Tuple4<Integer, Long, Integer, Integer>>("",
    ...
    class DSExtendReflexivKmerToArrayLoopFirstForRightSide implements MapPartitionsFunction<Row, Row>, Serializable {
        // ------ -------------------- Contig   forward
        // ------ -                    Kmer     reflected
                ...
                /* initiate a new capsule for the current sub-kmer group */
                    ...
                    if (tmpReflexivKmerExtendList.size() == 0) {
                        ...
                        for (int i = 0; i < tmpReflexivKmerExtendList.size(); i++) { // the tmpReflexivKmerExtendList is changing dynamically
                            ...
                            //else if (dynamicSubKmerComparator(s.getLong(0), tmpReflexivKmerExtendList.get(i).getLong(0))){
        ...
        public void singleKmerRandomizer(Row currentSubKmer) throws Exception {
            ...
            if (getReflexivMarker(currentSubKmer.getLong(1)) == 1) {
                ...
                if (currentSuffixLength>=3){ // k-mer might have one extension (or no extension), otherwise, it is contigs
                    randomReflexivMarker=1;
                }else{
                ...
                if (randomReflexivMarker == 2) {
                 ...
                 //   System.out.println("before : " + BinaryBlocksToString(currentSubKmerArray) + " " + BinaryBlocksToString(currentReflexivArray) + "\nnew: " + BinaryBlocksToString(newReflexivSubKmer) + " " + BinaryBlocksToString(newReflexivLongArray));
                ...
                int currentPrefixSize = currentKmerSizeFromBinaryBlockArray(currentReflexivArray);

                if (currentPrefixSize>=3){ // k-mer might have one extension, otherwise, it is contigs
                    randomReflexivMarker=1;
                }else{
                    randomReflexivMarker=2;
                }
        ...
        public void reflexivExtend(Row forwardSubKmer, Row reflexedSubKmer, int bubbleDistance) throws Exception {
            ...
            long[] forwardSubKmerArray =  seq2array(forwardSubKmer.getSeq(0));
            int forwardSubKmerLength = currentKmerSizeFromBinaryBlockArray(forwardSubKmerArray);

            long[] reflexedSubKmerArray = seq2array(reflexedSubKmer.getSeq(0));

            int reflexedPrefixLength = currentKmerSizeFromBinaryBlockArray(seq2array(reflexedSubKmer.getSeq(2)));
            int reflexedSubKmerLength = currentKmerSizeFromBinaryBlockArray(reflexedSubKmerArray);
            ...
            if (forwardSubKmerLength >= reflexedSubKmerLength){ // In reality, it is always forwardSubKmer longer than or equal to reflexedSubKmer
                ...
                longerSubKmer= forwardSubKmerArray;
            }else{
                newSubKmerLength=reflexedSubKmerLength;
                longerSubKmer = reflexedSubKmerArray;
            }
            ...
            if (forwardSuffixLength+reflexedPrefixLength>=3){ // k-mer might have one extension, otherwise, it is contigs
                randomReflexivMarker=1;
            }else{
                randomReflexivMarker=2;
            }
            ...
            if (randomReflexivMarker == 2) {
                ...
                System.out.println("before: " + BinaryBlocksToString(forwardSubKmerArray) + " " + BinaryBlocksToString(forwardSuffixArray) + " " + BinaryBlocksToString(reflexedPrefixArray) + " " + BinaryBlocksToString(reflexedSubKmerArray)
                        + "\nafter: " + BinaryBlocksToString(newReflexivLongArray) + " " + BinaryBlocksToString(newReflexivSubKmer));
                ...
                System.out.println("before: " + BinaryBlocksToString(forwardSubKmerArray) + " " + BinaryBlocksToString(forwardSuffixArray) + " " + BinaryBlocksToString(reflexedPrefixArray) + " " + BinaryBlocksToString(reflexedSubKmerArray)
                + "\nafter: " + BinaryBlocksToString(newForwardSubKmer) + " " + newForwardLongArray);
...
//                new Row("",
    ...
    class ContigBinarizerFromReducedToSubKmer implements MapPartitionsFunction<Row, Row>, Serializable{
        List<Row> kmerList = new ArrayList<Row>();
        Row units;
        String kmer;
...
*/
                ...
                attribute = buildingAlongFromThreeInt(2,-1,-1);
                ...
                int j;
                for (int i = currentContigSize-currentSubKmerSize; i < currentContigSize; i++) { // i is the orignial index
                    j=i-currentContigSize + currentSubKmerSize; // j is the new index
                    nucleotide = kmer.charAt(i);
                    if (nucleotide >= 256) nucleotide = 255;
                    nucleotideInt = nucleotideValue(nucleotide);
                    // forward kmer in bits
                    nucleotideInt <<= 2*(32-1-(j%31)); // shift to the left   [ATCGGATCC-,ATCGGATCC-]
                    nucleotideBinarySlot[j / 31] |=nucleotideInt ;
                ...
                nucleotideBinarySlot[currentSubKmerBlockSize-1] |= kmerEndMark;

                suffixBinaryArray = new long[(currentContigSize-currentSubKmerSize-1)/31+1];
                for (int i=0; i<currentContigSize-currentSubKmerSize;i++){ // i = old array index
                    char lastNt = kmer.charAt(i);
                    if (lastNt >= 256) lastNt = 255;
                    long lastNtLong = nucleotideValue(lastNt);
                    lastNtLong <<=  2*(32-1-(i%31));
                    suffixBinaryArray[i/31] |= lastNtLong;
                }
                ...
                // System.out.println("original: " + kmer + "\nNew suffix: " + BinaryBlocksToString(nucleotideBinarySlot) + " Puffix: " + BinaryBlocksToString(suffixBinaryArray) + "\nReflexiv marker: " + getReflexivMarker(attribute) + " left marker: " + getLeftMarker(attribute) + " right marker: " + getRightMarker(attribute));

                kmerList.add(
                        RowFactory.create(nucleotideBinarySlot, attribute, suffixBinaryArray)
                );
            }
